...
class DB:
    ...
    def save_session(user_id: int, session_string: str, phone: str, name: str):
        """Сохраняем StringSession в файл"""
    ...
    def load_session(user_id: int) -> Dict:
        """Загружаем StringSession из файла"""
        file = DATA_DIR / 'sessions' / f'{user_id}.json'
        if file.exists():
            try:
...
class Bot:
    def __init__(self):
        self.pending = {}  # Временные данные для авторизации
        self.clients = {}  # Активные клиенты {user_id: TelegramClient}
...
""")
    ...
    # ===== ОБРАБОТКА СООБЩЕНИЙ (авторизация) =====
    ...
    async def process_code(self, e, code: str):
        try:
            ...
            try:
                ...
                # ⭐ ГЛАВНОЕ: если вход заблокирован — просим ввести код заново
    ...
    async def load_sessions(self):
        ...
        for f in (DATA_DIR / 'sessions').glob('*.json'):
            try:
                ...
                # ⭐ ВОССТАНАВЛИВАЕМ КЛИЕНТА ИЗ STRING SESSION
