...
async def auto_ping():
    """Автоматический пинг каждые 4 минуты (Render не засыпает)"""
    ...
    while True:
        ...
        await asyncio.sleep(240)  # 4 минуты (Render засыпает через 15)
    ...
    async def process_code(self, e, code: str):
        try:
            ...
            try:
                ...
                await e.respond("❌ Код истек! /add заново")
                if uid in self.pending:
                    del self.pending[uid]
            except Exception as err:
                await e.respond(f"❌ Ошибка: {err}")
